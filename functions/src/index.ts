...
const ACCEPT_BASE_URL =
  process.env.ACCEPT_INVITE_BASE_URL || process.env.GCLOUD_PROJECT
...
async function sendInviteEmail(to: string, teamName: string, acceptLink: string): Promise<void> {
...
export const createInvite = functions.onCall(
  ...
  async (request): Promise<CreateInviteResult> => {
    ...
    const { teamId, email } = request.data as CreateInviteData;
...
export const acceptInvite = functions.onCall(
  ...
  async (request): Promise<AcceptInviteResult> => {
    ...
    const { token } = request.data as AcceptInviteData;
